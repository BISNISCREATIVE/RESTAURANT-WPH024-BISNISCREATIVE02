      ...
      <div>
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2">
            <Checkbox checked={nearby} onCheckedChange={(v) => setNearby(!!v)} />
            <span>Nearby</span>
          </label>
          <div className="text-sm text-muted-foreground">Max distance: {maxDistance ?? "Any"} km</div>
          <Slider value={[maxDistance ?? 5]} onValueChange={(v) => setMaxDistance(v?.[0] ?? null)} min={1} max={5} step={0.5} />
        </div>
      </div>
      <div>
        ...
        <div className="flex flex-col gap-2">
          {[5,4,3,2,1,0].map((v) => (
            <label key={v} className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="rating" checked={ratingMin===v} onChange={() => setRatingMin(v)} />
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 mx-auto max-w-6xl px-4 py-8 w-full">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl md:text-3xl font-extrabold">All Restaurant</h1>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filtered.map((r) => (
                  <div key={String(r.id)} className="relative">
                    <RestaurantCard data={r} onClick={() => { window.location.href = `/resto/${r.id}`; }} />
